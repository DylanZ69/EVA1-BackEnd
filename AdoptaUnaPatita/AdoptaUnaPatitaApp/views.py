...
def menu(request):
    rol = request.GET.get("rol", "usuario")  # ahora sí toma el rol del login
...
def enviar_solicitud(request):
    rol = request.GET.get("rol", "usuario")
    if rol != "usuario":
        # Solo los usuarios normales pueden enviar solicitudes
...
def gestionar_solicitud(request, id, accion):
    # Solo admin puede aceptar o rechazar solicitudes
    rol = request.GET.get("rol", "usuario")
    if rol != "admin":
        return redirect(f'/solicitudes/?rol={rol}')
